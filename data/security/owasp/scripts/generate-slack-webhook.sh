...
DEFAULT_WEBHOOK="https://hooks.slack.com/services/REDACTED/REDACTED/REDACTED"
...
validate_webhook_url() {
    ...
    if [[ ! "$url" =~ ^https://hooks\.slack\.com/services/T[A-Z0-9]+/B[A-Z0-9]+/[A-Za-z0-9]+ ]]; then
        ...
        echo "Expected format: https://hooks.slack.com/services/REDACTED/REDACTED/REDACTED..." >&2
        exit 1
    fi
}
...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    ...
    echo "Examples:"
    echo "  $0"
    echo "  $0 'https://hooks.slack.com/services/REDACTED/REDACTED/REDACTED'"
    echo
